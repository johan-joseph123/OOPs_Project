...
public class AdminDashboardPanel extends JPanel {
    ...
    public AdminDashboardPanel(RideShareMobileUI ui) {
        ...
        JPanel footer = new JPanel(new FlowLayout(FlowLayout.RIGHT, 15, 10)); footer.setBackground(new Color(245,245,245));
        JButton refreshBtn = new JButton("↻ Refresh"); 
        //JButton logoutBtn = new JButton("Logout");
        refreshBtn.addActionListener(e -> refreshTabs(tabbedPane));
        //logoutBtn.addActionListener(e -> ui.showScreen("login"));
        footer.add(refreshBtn); //footer.add(logoutBtn);
    ...
    private JPanel createApplicationsTab() {
        ...
        title.setFont(new Font("Segoe UI", Font.BOLD, 20)); title.setBorder(BorderFactory.createEmptyBorder(10,0,10,0));
        ...
        try {
            ...
            if (apps.isEmpty()) {
                ...
                empty.setFont(new Font("Segoe UI", Font.ITALIC, 16)); empty.setForeground(Color.GRAY);
                listPanel.add(empty);
            } else {
                for (DriverApplication da : apps) {
                    JPanel card = new JPanel(new BorderLayout());
                    card.setBackground(new Color(245,248,255)); card.setBorder(BorderFactory.createEmptyBorder(12,15,12,15));
                    ...
                    JPanel buttons = new JPanel(new FlowLayout(FlowLayout.RIGHT,8,10)); buttons.setOpaque(false);
                    JButton approve = new JButton("Approve"); JButton reject = new JButton("Reject");
                    ...
                    card.add(info, BorderLayout.CENTER); card.add(buttons, BorderLayout.EAST);
                    listPanel.add(card); listPanel.add(Box.createVerticalStrut(10));
                }
            }
        } catch (SQLException ex) {
            listPanel.add(new JLabel("Error: " + ex.getMessage()));
        }
        JScrollPane scroll = new JScrollPane(listPanel); scroll.setBorder(null); panel.add(scroll, BorderLayout.CENTER);
        return panel;
    }
    ...
    private void updateStatus(String id, String status) {
        try {
            ...
            if (ok) JOptionPane.showMessageDialog(this, "Driver " + id + " has been " + status + ".");
    ...
    private JPanel createRidesTab() {
        ...
        try {
            dao.RideDAO rideDAO = new dao.RideDAO();
            java.sql.Connection conn = dao.DBConnection.getConnection();
            java.sql.Statement st = conn.createStatement();
